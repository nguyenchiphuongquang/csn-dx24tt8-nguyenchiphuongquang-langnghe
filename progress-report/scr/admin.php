...
if (isset($_GET['delete_id'])) {
    ...
    header("Location: admin.php"); // T?i l?i trang sau khi xóa
}

// 2. X? LÝ THÊM D? LI?U M?I
if (isset($_POST['btn_add'])) {
    $name = $_POST['name'];
    $address = $_POST['address'];
    $history = $_POST['history'];
    // M?c d?nh gán location_id = 1 và category_id = 1 d? don gi?n hóa cho b?n demo
...
<body>
    <h2>Trang Qu?n Tr? H? Th?ng Làng Ngh?</h2>
    
    <!-- Form Thêm D? Li?u -->
    <div class="form-add">
        <h3>Thêm Làng Ngh? M?i</h3>
        <form method="POST" action="admin.php">
            <label>Tên Làng Ngh?:</label>
            <input type="text" name="name" required>
            
            <label>Ð?a ch?:</label>
            <input type="text" name="address" required>
            
            <label>L?ch s? hình thành:</label>
            <textarea name="history" rows="3" required></textarea>
            
            <button type="submit" name="btn_add">Thêm M?i</button>
        </form>
    </div>
    ...
    <table>
        ...
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                ...
                // Nút Xóa có dính kèm ID
                echo "<td><a class='btn-delete' href='admin.php?delete_id=" . $row['id'] . "' onclick='return confirm(\"B?n có ch?c ch?n mu?n xóa?\")'>Xóa</a></td>";
                echo "</tr>";
            }
        } else {
            echo "<tr><td colspan='4'>Chua có d? li?u</td></tr>";
        }
        ?>
    </table>
</body>
